...
class PasswordService {
  async reset({ id, currentPassword, newPassword }) {
    if (!id) throw new _AppError.AppError('The user id must be provided.');
    if (!currentPassword)
      throw new _AppError.AppError('The current password must be provided.');
    if (!newPassword)
      throw new _AppError.AppError('The new password must be provided.');
    ...
    const isValid = await (0, _bcryptjs.compare)(
      currentPassword,
      user.password,
    );
    if (!isValid)
      throw new _AppError.AppError('The current password is invalid.');
